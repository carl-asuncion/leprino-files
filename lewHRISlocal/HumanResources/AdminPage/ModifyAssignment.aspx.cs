...
namespace lewHRISlocal.HumanResources.AdminPage
{
    ...
    public partial class ModifyAssignment : System.Web.UI.Page
    {
        ...
        private void BindGrid1()
        {
            ...
            SqlDataAdapter sda2 = new SqlDataAdapter();
                ...
                " FROM [dbo].[View_GeneralistGrouped] WHERE NOT [Assigned_UserName] IS NULL", cnn);
        ...
        protected void mydatagrid_RowUpdating(object sender, System.Web.UI.WebControls.GridViewUpdateEventArgs e)
        {
            ...
            Label id = mydatagrid.Rows[e.RowIndex].FindControl("Department_Group_ID") as Label;
            //DropDownList status = mydatagrid.Rows[e.RowIndex].FindControl("txt_Status") as DropDownList;
            DropDownList ticket = mydatagrid.Rows[e.RowIndex].FindControl("txtUser") as DropDownList;
            //TextBox remarks = mydatagrid.Rows[e.RowIndex].FindControl("txt_Remarks") as TextBox;



            //updating the record  
            using (SqlCommand cmd = new SqlCommand("UPDATE [dbo].[HRMainDepartment] SET [Assigned_User_ID] =@ticket " +
                " WHERE [Department_Group_ID]="+ Convert.ToInt32(id.Text), cnn))
            {
                //if (ticket.SelectedValue == "Comp")
                if (string.IsNullOrEmpty(ticket.SelectedValue))
                {
                    cmd.Parameters.AddWithValue("@ticket", DBNull.Value);
                }
                else
                {
                    cmd.Parameters.AddWithValue("@ticket", ticket.SelectedValue);
                }
