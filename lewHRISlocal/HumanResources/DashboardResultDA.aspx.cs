...
namespace lewHRISlocal.HumanResources
{
    ...
    public partial class DashboardResultDA : System.Web.UI.Page
    {
        ...
        private void BindGrid1(string empName)
        {
            ...
            DataSet ds2 = new DataSet();
            SqlDataAdapter sda2 = new SqlDataAdapter();
            SqlCommand command = new SqlCommand("SELECT * FROM [View_3] WHERE [Employee_Status] LIKE '%" + empName + "%' AND ([Overall Status] = 'Initiated Disciplinary Action' OR " +
                "[Overall Status] = 'HRM Review for Disciplinary Action') ORDER BY [Date_Incident]", cnn);
